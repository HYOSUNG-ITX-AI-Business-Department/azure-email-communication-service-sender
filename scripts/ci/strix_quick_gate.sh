...
is_scannable_changed_file() {
	...
	if [[ ! "$changed_file" =~ \.(${SUPPORTED_SOURCE_FILE_EXTENSIONS})$ ]]; then
		return 1
	fi
...
extract_vulnerability_locations() {
...

	{
		...
		done < <(grep -Eo "(/workspace/[^[:space:]]+|[[:alnum:]_./-]+)\.(${SUPPORTED_SOURCE_FILE_EXTENSIONS}):[0-9]+" "$vuln_file" | sed 's/:[0-9][0-9]*$//' || true)

		while IFS= read -r location; do
			normalize_vulnerability_location "$location" || true
		done < <(grep -E "^[[:space:]]*(\*\*)?Target:(\*\*)?[[:space:]]*File:[[:space:]]*(/workspace/[^[:space:]]+|[[:alnum:]_./-]+)\.(${SUPPORTED_SOURCE_FILE_EXTENSIONS})([[:space:]]|$)" "$vuln_file" | sed -E 's/^[[:space:]]*(\*\*)?Target:(\*\*)?[[:space:]]*File:[[:space:]]*//' | sed -E 's/[[:space:]]+$//' || true)
	} | sort -u
}
